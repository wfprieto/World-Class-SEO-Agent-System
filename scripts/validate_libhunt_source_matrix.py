...
def _validate_source(source: dict[str, Any], source_ids: set[str]) -> list[str]:
    failures: list[str] = []
    missing = REQUIRED_SOURCE_FIELDS - set(source)
    if missing:
        return [f"source missing fields: {sorted(missing)}"]
...
def _validate_sources(matrix: dict[str, Any]) -> tuple[list[str], set[str]]:
    ...
    for source in sources:
        failures.extend(_validate_source(source, source_ids))
