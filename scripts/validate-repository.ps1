...
function Test-JsonFiles{Get-ChildItem -Path $Root -Recurse -Filter "*.json"|ForEach-Object{$jsonPath=$_.FullName;if(Test-GeneratedPath $jsonPath){return};try{Get-Content -Raw -LiteralPath $jsonPath|ConvertFrom-Json|Out-Null}catch{Add-Failure "Invalid JSON: $jsonPath - $($_.Exception.Message)"}}}
