...
function Test-GeneratedPath{param([string]$Path)$relative=$Path.Substring($Root.Length).TrimStart("\","/");return ($relative -match '^(outputs|dist|\.pytest_cache|\.mypy_cache|\.ruff_cache|\.clean-install|world_class_seo_agent_system\.egg-info)[\\/]')}
function Get-TrackedFiles{param([string]$Pattern)$paths=& git -C $Root ls-files -- $Pattern;if($LASTEXITCODE -ne 0){throw "git ls-files failed for $Pattern"};return $paths|ForEach-Object{Get-Item -LiteralPath (Join-Path $Root $_)}}
function Test-JsonFiles{Get-TrackedFiles "*.json"|ForEach-Object{$jsonPath=$_.FullName;try{Get-Content -Raw -LiteralPath $jsonPath|ConvertFrom-Json|Out-Null}catch{Add-Failure "Invalid JSON: $jsonPath - $($_.Exception.Message)"}}}
function Test-MarkdownLinks{$files=Get-TrackedFiles "*.md";$pattern='\[[^\]]+\]\(([^)]+)\)';foreach($file in $files){$content=Get-Content -Raw -LiteralPath $file.FullName;[regex]::Matches($content,$pattern)|ForEach-Object{$target=$_.Groups[1].Value;if($target -match '^(https?:|mailto:|#)'){return};$clean=($target -split '#')[0];if([string]::IsNullOrWhiteSpace($clean)){return};$combined=Join-Path -Path $file.DirectoryName -ChildPath $clean;if(-not(Test-Path -LiteralPath $combined)){Add-Failure "Broken markdown link in $($file.FullName): $target"}}}}
...
Test-PythonValidator "scripts/validate_autonomous_seo_expansion_program.py"
Test-PythonValidator "scripts/show_autonomous_ruff_import_diff.py"
...
Test-PythonValidator "scripts/validate_open_issue_remediation.py"
Test-PythonValidator "scripts/validate_private_conduct_intake.py"
Test-PythonValidator "scripts/validate_specialist_depth.py" @("--mutations")
Test-PythonValidator "scripts/validate_capability_certification.py"
Test-PythonValidator "scripts/validate_scheduled_maintenance.py"
