"""Canonical outbound URL safety for every executable layer.

This is the single source of truth for SSRF and privacy hazards on any URL the
system fetches, renders, or sends to a provider.
"""
...
def _parse_target(url: object) -> tuple[urllib.parse.SplitResult, str, str, int | None]:
...
def _assert_public_host(host: str, port: int, resolver: Resolver) -> None:
    ...
    for address in addresses:
        ...
        if not resolved.is_global:
            raise ValueError(
                f"URL host resolves to a non-public address: {resolved.compressed}"
            )
...
def validate_public_url(url: str, *, resolver: Resolver = socket.getaddrinfo) -> str:
    ...
    query_keys = {
        key.lower() for key, _ in urllib.parse.parse_qsl(parsed.query, keep_blank_values=True)
    }
