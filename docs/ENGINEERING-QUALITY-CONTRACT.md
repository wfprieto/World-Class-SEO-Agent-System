...
Every repository-dependent command and release attestation must immediately follow the canonical observational source-integrity gate. The gate runs the captured absolute interpreter with `-I -E -S`, requires a structured JSON `PASS`, independently hashes tracked worktree content, compares the full staged index and file modes with `HEAD`, proves `HEAD` equals the event SHA, and rejects untracked import-shadowing source. This remains effective when Git marks a file `assume-unchanged`. Workflow command-channel writes through `GITHUB_ENV` or `GITHUB_PATH`, inherited execution controls, containers, services, self-hosted runners, and runner matrices outside the exact GitHub-hosted allowlist fail structurally. Rollback jobs prove the candidate immediately before their intentional tree replacement and then use their separate exact-baseline tree proof.

The top-level triggers, permissions, concurrency, exact job set, job dependencies and conditions, complete matrix objects, runner assignments, job permissions, mandatory-step condition/error/shell metadata, immutable action sequence and inputs, checkout/setup bootstrap order, aggregate result assertions, attestations, and rollback receipts form one finite canonical contract. Any structural drift, including a non-mapping job, fails closed. Source proofs execute with an absolute interpreter path captured before dependency installation and with Python isolation flags.

Command-channel protection is deliberately bounded static analysis, not a claim to interpret arbitrary runtime shell semantics. It rejects inherited dangerous environment keys and a finite case-insensitive lexical normalization catalog for direct and reviewer-reproduced quoted, concatenated, and formatted spellings of `GITHUB_ENV` and `GITHUB_PATH`. Dynamic names outside that catalog are not claimed; exact step metadata and fresh adjacent source proofs remain independent controls. Rollback jobs enter baseline mode only through the exact canonical rollback operation, then require a fresh exact-tree and worktree proof immediately before every baseline install, validation, test, receipt-sealing, and artifact boundary.
