...
`seoctl` is the outer composition layer and may assemble runtime, adapter, and integration components. All other cross-layer imports fail unless the architecture contract names the exact source module, exact target module, accountable owner, rationale, and removal phase. Absolute and relative imports, including package `__init__` imports, participate in the same cycle graph. The validator also rejects stale exceptions, missing packages, and new modules that directly import a declared network or browser client, dynamically import a declared client by a literal name, or launch a statically identifiable network command without explicit registration. This is an import-and-literal-call egress inventory, not whole-program data-flow proof; indirect calls and dynamically constructed command strings remain the responsibility of security review.
