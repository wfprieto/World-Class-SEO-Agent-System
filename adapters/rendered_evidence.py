...
def _redacted_mapping(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(sanitized, dict):
        raise TypeError("rendered evidence redaction must preserve mapping shape")
    return sanitized
...
def merge_rendered(data: dict[str, Any], rendered: dict[str, Any], raw_html: str) -> None:
        ...
        _redacted_mapping(
            {
                ...
                "console_errors": [
                    sanitize_text_for_evidence(item) for item in rendered.get("console_errors", [])
                ],
